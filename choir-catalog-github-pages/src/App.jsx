...
const CHANGE_REQUEST_ROOT = 'https://github.com/mikemaksimchuk/choir-catalog/issues/new'
const ADMIN_SESSION_KEY = 'choir-catalog-editing-unlocked'
// GitHub Pages cannot keep a browser-delivered secret. This hash is a deterrent only;
// GitHub collaborator authorization is what protects the nightly database update.
const ADMIN_PASSWORD_SHA256 = '86bcfbb90a3dbdb68dac16d1e12008e0ffb43117a0da0ea102c84d626dd2cf8b'
...
function createChangeRequestUrl({ operation, originalCatalogNumber, entry }) {
  const action = operation === 'add' ? 'Add' : 'Edit'
  const title = `[Catalog ${action}] ${entry['Catalog Number']} - ${entry.Title}`
  const payload = { operation, originalCatalogNumber, entry }
  const body = [
    '<!-- choir-catalog-change:v1 -->',
    `## ${action} choir catalog entry`,
    '',
    'Please review the information below, then click **Submit new issue**. The nightly catalog workflow will apply requests submitted by an authorized repository owner or collaborator.',
    '',
    '> Do not change the JSON block below. Close this page without submitting if you do not want to request the change.',
    '',
    '```json',
  ...
  return `${CHANGE_REQUEST_ROOT}?${params.toString()}`
}
...
function EntryForm({ initialEntry, mode }) {
      ...
      <section className="form-panel">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="request-note">
            ...
            <p><strong>Final authorization happens on GitHub.</strong> Continuing opens a prepared change request. Sign in to GitHub, review it, and click <em>Submit new issue</em>. Authorized requests are applied and backed up at midnight.</p>
          </div>
          {requestUrl && (
            <div className="request-ready" role="status">
              <Check aria-hidden="true" size={18} />
              <p>The GitHub request page was opened. If it did not open, <a href={requestUrl} target="_blank" rel="noreferrer">continue to GitHub here <ExternalLink aria-hidden="true" size={15} /></a>.</p>
            </div>
          )}
...
function AuthDialog({ open, onClose, onAuthenticated }) {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (passwordHash !== ADMIN_PASSWORD_SHA256) throw new Error('That password was not accepted.')
  ...
  return (
    <div className="dialog-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section ref={dialogRef} className="auth-dialog" role="dialog" aria-modal="true" aria-labelledby="auth-title" aria-describedby="auth-description">
        ...
        <p id="auth-description">Enter the catalog password to open the editing form. Because this site is public and static, GitHub sign-in and collaborator authorization are still required to submit a change.</p>
        <form onSubmit={handleSubmit}>
          <label className="field">
            <span>Password</span>
...
export default function App() {
  ...
  if (loading && route.page !== 'catalog') {
    ...
    content = authenticated ? <EntryForm mode="add" /> : <main className="page-shell"><EmptyState filtered /></main>
  } else if (route.page === 'edit') {
    content = authenticated && currentEntry
      ? <EntryForm mode="edit" initialEntry={currentEntry} />
